...
> **最后更新**: 2026-05-02
> **版本**: v1.2（经多轮审查修复，对照 v2026.05.01 部署 prompt 查缺补漏）
...
2. **新增 `computeTranscriptTokenTotals(transcriptPath)`**:
   ...
   - 累加所有 `message.usage.inputTokens + outputTokens`
...
- delta = event.tokens（直接信任，不做减法）
- `_flush()`: 30s 间隔，写入 token-stats.json
- 日期切换: en-CA locale (YYYY-MM-DD)，上海时区
- `isAlive()`: 120s 无 flush 则判定死亡

**⚠️ 已知 bug（已修复，新版本可能不同）**:
...
| | 修改九 | 修改 `_flush()` Math.max 合并 + 保留 daemon 字段 |
| | 修改十 | 新增 `_lastFlushMs` + `isAlive()` 健康检查 |
...
### 日期格式
- 使用 `toLocaleDateString('en-CA', {timeZone: 'Asia/Shanghai'})`
- 返回 `YYYY-MM-DD` 格式
- **不要用 `.split('/')`**（en-CA 返回横杠不是斜杠）

### isAlive 阈值
- TokenAggregator: 120s（30s flush × 4 倍安全系数）
