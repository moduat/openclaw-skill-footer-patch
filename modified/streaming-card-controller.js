...
function resolveContextWindowFromConfig(cfg, modelName) {
    try {
        ...
        if (!providers || !modelName) return undefined;
...
function calcModelCost(inputTokens, outputTokens, cacheRead, model, cfg) {
    try {
        const providers = cfg?.models?.providers;
        if (!providers || !model) return 0;
        for (const provider of Object.values(providers)) {
            if (!provider?.models) continue;
            for (const m of provider.models) {
                if (!m?.id) continue;
                if (m.id === model || model.endsWith('/' + m.id) || m.id.endsWith('/' + model) || model.includes(m.id)) {
                    const cost = m.cost;
                    if (!cost) return 0;
                    const inputCost = (inputTokens || 0) / 1_000_000 * (cost.input || 0);
                    const outputCost = (outputTokens || 0) / 1_000_000 * (cost.output || 0);
                    const cacheCost = (cacheRead || 0) / 1_000_000 * (cost.cacheRead || 0);
...
function computeGlobalTokenStats(cfg) {
    try {
        const { existsSync, readFileSync } = require('node:fs');
        const path = require('path');
        const statsPath = path.join(require('os').homedir(), '.openclaw', 'token-stats.json');
        if (!existsSync(statsPath)) return {};
        const data = JSON.parse(readFileSync(statsPath, 'utf8'));
        return {
            todayTokens: typeof data.todayTokens === 'number' ? data.todayTokens : undefined,
            monthTokens: typeof data.monthTokens === 'number' ? data.monthTokens : undefined,
...
class StreamingCardController {
    ...
    _transcriptFirstTokenTs = null;
    _transcriptStartTime = null;
    ...
    needsFooterMetrics() {
        ...
        return footer.tokens || footer.cache || footer.context || footer.model || footer.cost || footer.todayTokens || footer.monthTokens;
    }
    async getFooterSessionMetrics() {
        try {
            const cfg = this.deps.cfg;
            const sessionStorePath = cfg.sessions?.store ?? cfg.session?.store;
            ...
            // Try agent-specific store first (e.g. agents/niumaxia/sessions/sessions.json)
            const { existsSync } = require('node:fs');
            const path = require('path');
            const os = require('os');
            const effectiveAgentId = agentId || defaultAgentId;
            const agentStorePath = path.join(os.homedir(), '.openclaw', 'agents', effectiveAgentId, 'sessions', 'sessions.json');
            const configStorePath = config_runtime_1.resolveStorePath(sessionStorePath);
            const storePath = existsSync(agentStorePath) ? agentStorePath : configStorePath;
            ...
            // If not found in agent store, try config default store
            if (!entry && storePath !== configStorePath) {
                const fallbackStore = config_runtime_1.loadSessionStore(configStorePath);
                for (const candidate of candidateKeys) {
                    const val = fallbackStore[candidate];
                    if (val && typeof val === 'object') {
                        entry = val;
                        matchedKey = candidate;
                        break;
            ...
            if (!entry) {
                log.debug('footer metrics: session entry missing', {
            ...
            const total = computeTranscriptTokenTotals(
                this.deps?.paths?.sessionTranscript
            );
            if (total) {
                log.debug('footer metrics: transcript totals computed', {
                    input: total.input,
                    output: total.output,
                });
            }
            const ftl = typeof this._transcriptFirstTokenTs === 'number' && typeof this._transcriptStartTime === 'number'
                ? this._transcriptFirstTokenTs - this._transcriptStartTime : undefined;
            const metrics = collectFooterMetrics(entry, {
                firstTokenLatencyMs: ftl,
            });
            if (total) {
                metrics.inputTokens = total.input;
                metrics.outputTokens = total.output;
            }
            const contextWindow = resolveContextWindowFromConfig(cfg, metrics.model ?? '');
            const model = metrics.model ?? entry.model;
            const inputTokens = metrics.inputTokens ?? entry.inputTokens;
            const outputTokens = metrics.outputTokens ?? entry.outputTokens;
            const cacheRead = metrics.cacheRead ?? entry.cacheRead;
            metrics.cost = calcModelCost(inputTokens, outputTokens, cacheRead, model, cfg);
            const { todayTokens, monthTokens, dateKey } = computeGlobalTokenStats(cfg);
            metrics.todayTokens = todayTokens;
            metrics.monthTokens = monthTokens;
            metrics.dateKey = dateKey;
            metrics.contextWindow = contextWindow;
            metrics.agentName = agentId;
            log.debug('footer metrics: resolved', {
                sessionKey: this.deps.sessionKey,
                matchedKey,
                model: metrics.model,
    ...
    constructor(deps) {
        this.deps = deps;
        this._transcriptStartTime = Date.now();
    ...
    async onDeliver(payload) {
        ...
        if (this._transcriptFirstTokenTs == null) this._transcriptFirstTokenTs = Date.now();
