...
function formatFooterRuntimeSegments(params) {
    ...
    if (footer?.context && metrics) {
        const total = typeof metrics.totalTokens === 'number' ? Math.max(0, metrics.totalTokens)
            : (typeof metrics.inputTokens === 'number' && typeof metrics.outputTokens === 'number')
                ? Math.max(0, metrics.inputTokens + metrics.outputTokens)
                : undefined;
        const ctx = typeof metrics.contextTokens === 'number' ? Math.max(0, metrics.contextTokens)
            : (typeof metrics.contextWindow === 'number' ? metrics.contextWindow : undefined);
        if ((total != null || (typeof metrics.inputTokens === 'number' && metrics.inputTokens >= 0)) && ctx != null) {
    ...
    if (footer?.cost && typeof metrics?.cost === 'number' && metrics.cost > 0) {
        const costStr = metrics.cost < 0.0001 ? metrics.cost.toExponential(2) : `$${metrics.cost.toFixed(4)}`;
        detailZh.push(`💸 ${costStr}`);
        detailEn.push(`💸 ${costStr}`);
    }
    if (typeof metrics?.firstTokenLatencyMs === 'number' && metrics.firstTokenLatencyMs > 0) {
        const sec = (metrics.firstTokenLatencyMs / 1000).toFixed(2);
...
function buildCompleteCard(params) {
    ...
    // Footer meta-info: multi-line
    // Line 1: 🪙Token今/月
    // Line 2: status · elapsed · model
    // Line 3: tokens · cache · context · cost · firstToken
    ...
    const footerZhLines = [];
    const footerEnLines = [];
    // Line 1: Token stats (global)
    const showGlobalTokens = footer?.todayTokens || footer?.monthTokens;
    if (showGlobalTokens && footerMetrics) {
        const { todayTokens, monthTokens } = footerMetrics;
        if (todayTokens != null || monthTokens != null) {
            const today = todayTokens != null ? compactNumber(todayTokens) : '—';
            const month = monthTokens != null ? compactNumber(monthTokens) : '—';
            const now = new Date();
            ...
            const tokenLine = `🪙Token今/月: ${today}丨${month} · ${ts}`;
            footerZhLines.push(tokenLine);
            footerEnLines.push(tokenLine);
        }
    }
    // Separator
    if (footerZhLines.length > 0 && (fp.primaryZh.length > 0 || fp.detailZh.length > 0)) {
        footerZhLines.push('──────────────────');
        footerEnLines.push('──────────────────');
    }
    // Line 2: status · elapsed · model
